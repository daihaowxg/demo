...
### 什么是数据库游标 (Database Cursor)？

**游标 (Cursor)** 是数据库系统在内存中保留的一个区域，用于存放 SQL 语句执行的结果集。你可以把它想象成一个**指针**，它指向查询结果集中的某一行数据。

*   **作用**: SQL 是一种集合操作语言，一次通常处理多行数据。而应用程序（如 Java）通常需要逐行处理数据。游标就在这两者之间架起桥梁，允许应用程序遍历结果集，一次访问一行。
*   **JDBC 中的体现**: 在 JDBC 中，`ResultSet` 本质上就是对数据库游标的封装。当你调用 `resultSet.next()` 时，底层的游标就向下移动一行。
*   **资源消耗**: 游标非常昂贵。维护一个游标意味着数据库要在内存中保持这部分查询结果的状态。如果打开太多游标而不关闭，会迅速消耗数据库内存（PGA in Oracle）或达到最大打开游标数限制，导致数据库报错。

### JDBC 查询大批量数据时是如何交互的？

这取决于数据库驱动的**默认配置**和你的**fetchSize**设置。

#### 1. 游标在哪里？
是的，逻辑游标通常维护在**数据库服务器端**。它记录了当前读取到了哪一行。

#### 2. 一次性拿完还是分批拿？
*   **默认行为（MySQL）**: MySQL JDBC 驱动默认非常“贪婪”，它会**一次性**把所有结果集读入到客户端（Java 进程）的内存中。
    *   **优点**: 读取完后释放数据库锁快。
    *   **缺点**: 如果查询结果有几百万行，会导致 Java 进程 **OOM (Out Of Memory)**。
*   **默认行为（Oracle/PostgreSQL）**: 通常默认有一定的 `fetchSize`（例如 10 行），即第一次只从网络拉取 10 条数据。当你遍历完这 10 条调用 `next()` 时，驱动会自动再去服务器取下一批。

#### 3. 如何控制（流式查询）？
你可以通过 `statement.setFetchSize(int rows)` 来控制。
*   **Oracle/Postgres**: 设置 `fetchSize=1000`，意味着每次网络通信取 1000 条。
*   **MySQL**: 比较特殊，要实现流式读取（不全量加载），通常需要：
    1.  `statement.setFetchSize(Integer.MIN_VALUE)`
    2.  确保 `Forward Only` 游标类型。
    这样 MySQL 才会一行一行（或一批一批）地通过网络传输数据，而不是一次性塞满内存。
