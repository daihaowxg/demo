...
### 二级缓存 (L1+L2) 适用场景
二级缓存增加了系统复杂度（一致性维护难），仅建议在遇到**Redis 瓶颈**时使用：
1.  **极度热点数据 (Hot Key)**：单个 Key 的 QPS 极高（如数万），导致 Redis 网卡打满或 CPU 瓶颈。本地缓存可拦截绝大部分请求。
2.  **极低延迟要求**：要求微秒级响应，Redis 的毫秒级网络 I/O (RTT) 无法满足。
3.  **大对象数据**：缓存对象很大（如 HTML/JSON），频繁从网络拉取会消耗巨大带宽。
4.  **改少读多**：数据极少变更，能容忍短时间的一致性差异（因为分布式环境下的本地缓存清除很难做到强一致）。
